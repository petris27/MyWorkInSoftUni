...
while command != "Lumpawaroo":
    if "|" in command:
        ...
        for value in force_side_dic.values():
            if force_user in value:
                user_is_part_of_the_force = True
                break
        if not user_is_part_of_the_force:
           if force_side not in force_side_dic.keys():
               force_side_dic[force_side] = []
           force_side_dic[force_side].append(force_user)
    elif "->" in command:
    ...
    command = input()
for force_side, force_users in force_side_dic.items():
